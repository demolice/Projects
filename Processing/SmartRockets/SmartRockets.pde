...
void setup() {
  ...
  setUpBoxes(); //metoda pro nastavení Textboxu
}
...
void draw() {
  ...
  if (ticketCount == lifespan) {
    ...
    if (writer) {
      /*
      output.println("Generation number: " + generationNumber);
      output.println("Hits this round: " + hitsThisRound);
      output.println("All time best hits: " + allTimeBestHits);
      output.println("Best time: " + bestTime);
      output.println("All time best time: " + allTimeBestTime);
      output.println("----------------------");
      */
      ...
      //output.println("schema:\tgeneration number\thits this round\tall time best\tbest time\tall time best time");
    }
...
void updateData(Textbox t) {
  switch (t.name) {
    ...
    try {
      ...
      //t.Text = Integer.toString(rocketsCount);
      println(e);
    }
    break;
    case("randomOnesBox"):
    t.repairSize();
    //println("random ones box: " + t.Text);
    ...
    if (!t.Text.equals("")) {
      if (!(t.Text.substring(t.Text.length() - 1).equals("0") || t.Text.substring(t.Text.length() - 1).equals("."))) {
        //if (!(t.Text.equals("") || t.Text.equals("0") || t.Text.equals("0.") || t.Text.equals("0.0") || t.Text.equals("0.00"))) {
        try {
